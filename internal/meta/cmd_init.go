...
func runInit(cmd *cobra.Command, _ []string) error {
	...
	if nonInteractive {
		...
		fmt.Fprintln(os.Stderr, "Error: `cojira init` is interactive. Edit .env manually instead.")
		return &exitError{Code: 3}
	}
	...
	if !output.IsTTY(int(os.Stdin.Fd())) || !output.IsTTY(int(os.Stdout.Fd())) {
		...
		fmt.Fprintln(os.Stderr, "Hint: Edit .env manually, then run `cojira doctor`.")
		return &exitError{Code: 3}
	}
	...
	if err := writeEnvFile(outPath, values); err != nil {
		return err
	}
